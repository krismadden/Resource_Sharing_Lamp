...
const int maxBrightness = 5; //this can be any number - it's the number of steps between dimmest and brightest.
const int interval = 1;
const boolean paris = true;
const int ledsPerUnit = 12; // this is NUM_LEDS / maxBrightness. 
...
//commented out setup
  ...
  Serial.print("adafruitFeed4: ");
  Serial.println(feed4Data);
...
//when the + button is pressed and it was previouslly released
  if (digitalRead(BRIGHTNESS_UP) == LOW && upButtonDown == false) {
    ...
    if(totalUnits >= maxBrightness && brightness < maxBrightness){
